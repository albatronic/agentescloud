    ...
    public function getListColumns() {
        ...
        if ($this->getNode('columns')) {
            foreach ($this->getNode('columns') as $key => $value) {
                if ((strtoupper($value['list']) == 'YES') or ( $value['list'] == 1)) {
                    if ($columnas !== '') {
                        $columnas .= ", ";
                    }
                    $columnas .= (string) $key;
    ...
    public function getRules() {
        ...
        if (is_array($columns)) {
            foreach ($columns as $key => $value) {
                if ((strtoupper($value['validator']['nullable']) == 'NO') || ($value['validator']['nullable'] == false)) {
                    $rules[$key] = array(
                        'title' => $value['title'],
                        'type' => $value['validator']['type'],
                        'length' => $value['validator']['length'],
