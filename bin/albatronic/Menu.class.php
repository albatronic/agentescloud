...
class Menu {
    ...
    public function getOpciones($idPerfil = '') {
        ...
        if (is_resource($em->getDbLink())) {
            if ($idPerfil != '')
                ...
                $query = "select CodigoApp as Id,NombreApp as Value from {$tablaApps} order by Id";
            //echo $query;
            $em->query($query);
            $rows = $em->fetchResult();
            $em->desConecta();
    ...
    public function getSubopciones($codigoApp, $idPerfil = '') {
        ...
        if (is_resource($em->getDbLink())) {
            if ($idPerfil != '')
                ...
                $query = "select NombreModulo as Id, Titulo as Value from {$tablaModulos} where (CodigoApp='{$codigoApp}' and Nivel='1') order by Id;";
            //echo $query;
            $em->query($query);
            $rows = $em->fetchResult();
            $em->desConecta();
    ...
    private function creaArray() {
        ...
        if (is_resource($dblink)) {
            $query = "select DISTINCT t1.IDOpcion,t2.Script,t2.Titulo from AgtPermisos as t1, AgtMenu as t2 where (t1.IDPerfil='" . $this->idPerfil . "' and t1.IDOpcion=t2.IDOpcion) order by t2.Orden";
            $em->query($query);
            $rows = $em->fetchResult();
            foreach ($rows as $row) {
                $query = "select t2.* from AgtPermisos as t1,AgtSubmenu as t2 where (t1.IDPerfil='" . $this->idPerfil . "' and t1.IDOpcion='" . $row['IDOpcion'] . "' and t1.IDSubopcion<>0 and t1.IDOpcion=t2.IDOpcion and t1.IDSubopcion=t2.Id) order by t2.Orden;";
                $em->query($query);
