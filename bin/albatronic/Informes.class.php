...
class Informes extends Controller {
    ...
    public function __construct($request) {
        ...
        $file = $_SERVER['DOCUMENT_ROOT'] . $_SESSION['appPath'] . "/docs/docs{$_SESSION['emp']}/informes/informes.yml";
        $existe = file_exists($file);
        if (!$existe) {
            $file = $_SERVER['DOCUMENT_ROOT'] . $_SESSION['appPath'] . "/docs/informes/informes.yml";
            $existe = file_exists($file);
        }
    ...
    private function getFormatsReports() {
        ...
        if (is_array($this->reports['informes'])) {
            $perfilUsuario = $_SESSION['usuarioPortal']['IdPerfil'];
            foreach ($this->reports['informes'] as $value) {
