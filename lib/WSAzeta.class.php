...
/**
 * Description of WSAzeta
 *
 * @author Sergio Pérez <info@albatronic.com>
 */
class WSAzeta extends WebService {

    /**
     * El id de Azeta de la tabla de distribuidoras
     * @var integer
     */
    static $idDistribuidora = 2;
    ...
    static private function postProcesoBusqueda($resultado) {
        ...
        foreach ($resultado as $key => $item) {
            ...
            if (self::$resultado[$keyISBN]->Precio_libre == 'S') {
    ...
    /**
     * Hace reserva en Azeta y marca cada linea
     ...
     * Si falta stock de algún producto, se anula el pedido a Azeta
     * 
    ...
    static function reservar(array $articulos) {
        ...
        if ($status == 'OK') {
            ...
            LogOperaciones::anota(
                    "RESERVAR EN AZETA", $status, sfYaml::dump($articulos), print_r(self::$resultado, true), self::$idDistribuidora, self::$resultado[0]->cod_pedido);
            ...
            if ($hayDeTodo) {
                ...
                foreach (self::$resultado as $item) {
                    if ($item->can_pedida == $item->can_reservada) {
                        $lineas->queryUpdate(array('PedidoDistribuidora' => $item->cod_pedido, 'IdEstado' => 1), "Id='{$item->idLineaPedido}' and IdEstado='0'");
                ...
                // No hay suficiente stock. Se le comunica a Azeta que no queremos el pedido
                // y se devuelve el array vacio para no enviarle nada a VIPS
                self::anulaPedido("ANULAR PEDIDO EN AZETA. STOCK INSUFICIENTE", self::$resultado[0]->cod_pedido);
                self::$resultado = array();
            }
        } else {
            LogOperaciones::anota(
                    "RESERVAR EN AZETA. Fallo en la llamada", $status, sfYaml::dump($articulos), print_r(self::$resultado, true), self::$idDistribuidora, self::$resultado[0]->cod_pedido);
        }
    ...
    /**
     * Hace una llamada a Azeta indicando que anule un pedido
     * 
     * @param integer $codigoPedido Código de pedido de Azeta
     * @return type
     */
    static function anulaPedido($mensaje, $codigoPedido) {
        ...
        LogOperaciones::anota(
                $mensaje, $status, $codigoPedido, print_r($resultado, true), self::$idDistribuidora, $codigoPedido);
    ...
    /**
     * Envía confirmación de pedido a Azeta,
     * indicando para cada código de pedido Azeta el
    ...
    static function confirmar(array $pedido) {
        ...
        LogOperaciones::anota("ENVIO CONFIRMACION PEDIDOS A AZETA", $status, print_r($pedido, true), print_r($resultado, true), self::$idDistribuidora, $pedido['orderWeb'], $pedido['orderVips']);
