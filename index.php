 ...
 *
 * El apppath puede estar compuesto de varias subcarpetas. Ej:
...
$_SESSION['debug'] = $config['debug'];
$_SESSION['enviroment'] = $config['enviroment'];
$_SESSION['export_types'] = $config['export_types'];
$_SESSION['emp'] = 1;
$_SESSION['project']['title'] = $app['name'];
$_SESSION['app'] = $app;
...
if ($rq->isOldBrowser()) {
    ...
    if (!isset($_SESSION['usuarioPortal']) && ($controller !== "Index") && ($action !== "FastLogin")) {
        // No está logeado
        $controller = "Index";
        if ($action != 'Recordar') {
            $action = "Login";
